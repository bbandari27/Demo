...
def disable_cloudtrail_in_all_regions(account_id):
    ...
    if assumed_role:
        ...
        for region in regions:
            try:
                response = cloudtrail_client.describe_trails(trailNameList=['EllucianTrailV2'], includeShadowTrails=False, region=region)
                # print(response)
                ...
                if 'trailList' in response and len(response['trailList']) > 0:
                    ...
                    if trail_status:
                        ...
                        print(f"CloudTrail 'EllucianTrailV2' is already disabled for Account {account_id} in region: {region}.")
                else: 
                    print(f"CloudTrail 'EllucianTrailV2' not found for Account {account_id} in region: {region}.")

            except Exception as e:
                print(f"Error checking CloudTrail 'EllucianTrailV2' in region: {region}. Error: {str(e)}")
